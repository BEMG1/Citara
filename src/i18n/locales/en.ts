import type { TranslationDictionary } from '../types';

export const en: TranslationDictionary = {
  cancel: 'Cancel',
  save: 'Save',
  delete: 'Delete',
  copy: 'Copy',
  warning: 'Warning',
  copied: 'Copied!',

  appTitle: 'Citara',
  appSubtitle: 'Academic text generator and formatter',
  themeToggleLight: 'Switch to light mode',
  themeToggleDark: 'Switch to dark mode',
  languageToggle: 'Change language',

  formatSelectorTitle: 'Citation Format',
  formatSelectorSubtitle: 'Select the format you want to use for your references',
  formatAPA7: 'APA 7th Edition',
  formatAPA7Desc: 'Current standard. Recommended for most papers.',
  formatAPA6: 'APA 6th Edition',
  formatAPA6Desc: 'Previous version, still used in some institutions.',
  formatIEEE: 'IEEE Format',
  formatIEEEDesc: 'Numeric style, commonly used in engineering and sciences.',
  formatUPEL: 'UPEL Norms',
  formatUPELDesc: 'Norms from the Universidad Pedagógica Experimental Libertador for theses and degrees.',
  builtInFormats: 'Built-in Formats',
  customFormats: 'Custom Formats',
  createCustomFormat: 'Create Custom Format',
  loginRequiredFeature: 'You must login or create an account to use this feature',

  addReference: 'Add IReference',
  noReferences: 'No references yet.',
  noReferencesHint: 'Click the button above to start.',
  sortAZ: 'A→Z',
  sortAZTooltip: 'Sort A→Z by author',
  showingExportOrder: 'Showing export order',
  newReference: 'New reference',
  incompleteRef: 'Incomplete',
  copyRefTooltip: 'Copy reference',
  deleteRefTooltip: 'Delete reference',

  sourceType: 'Source Type',
  typeBook: 'Book',
  typeArticle: 'Journal Article',
  typeWebsite: 'Website',
  typeVideo: 'Video (YouTube, etc.)',

  authors: 'Author(s)',
  authorsHint: '(E.g. Doe, J., & Smith, A.)',
  authorsPlaceholder: 'Doe, J.',
  year: 'Year',
  yearPlaceholder: '2023',
  yearErrorFormat: 'Invalid format (e.g., 2023 or n.d.)',
  yearErrorFuture: 'Year cannot be in the future',
  title: 'Title',
  titlePlaceholder: 'Title of the work',
  publisher: 'Publisher',
  publisherPlaceholder: 'Publisher Name',
  journalName: 'Journal Name',
  journalPlaceholder: 'Journal of Psychology',
  volume: 'Volume',
  issue: 'Issue',
  pages: 'Pages',
  doi: 'DOI',
  doiHint: 'without https://doi.org/ prefix',
  url: 'URL',
  siteName: 'Website Name',
  siteNamePlaceholder: 'Wikipedia, WHO, etc.',
  channelName: 'Channel or Video Author',
  channelPlaceholder: 'YouTube channel name',

  preview: 'Preview',

  documentTitle: 'Main Document',
  documentSubtitle: 'Write your text and select fragments to insert citations.',
  exportWord: 'Export to Word',
  exportBtn: 'Export',
  exportDocument: 'Export document',
  exportDocxTitle: 'Export as Microsoft Word',
  exportDocxDesc: 'Generates a Word-compatible file maintaining all the structure, margins, and indentations of the chosen format.',
  exportPdfTitle: 'Export as PDF Document',
  exportPdfDesc: 'Ideal for submitting or printing directly. Guarantees that typography and visual design remain completely unaltered.',
  copyText: 'Copy text',
  placeholderText: 'Write or paste your text here...',

  // Export Warning Modal
  warningTitle: 'Incomplete References',
  warningMessage: 'There are references in your list missing important information (yellow marks).',
  exportAnyway: 'Export anyway',
  reviewReferences: 'Review References',

  // Document Editor Tooltips and UI
  associatedSource: 'Associated Source',
  linkedReference: 'Linked reference',
  removeLink: 'Remove',
  associateReference: 'Associate reference...',
  availableSources: 'AVAILABLE SOURCES',
  noReferencesCreated: 'No references created',
  addSourcesFromPanel: 'Add sources from the side panel.',
  noAuthor: 'No author',
  noTitle: 'No title',
  onlyDocxAllowed: 'Only .docx files are allowed',
  errorReadingWord: 'There was an error reading the Word document.',
  uploadDocx: 'Upload .docx',
  loading: 'Loading...',
  orDragDrop: 'or drag the file to the editor',
  clearDocumentPrompt: 'Clear all document text?',
  clearDocument: 'Clear',
  dropDocxHere: 'Drop .docx here',
  heading1: 'Heading 1',
  heading2: 'Heading 2',
  heading3: 'Heading 3',
  paragraph: 'Paragraph',

  // Cover Page
  coverPageToggle: 'Include cover page',
  coverPageTooltip: 'Configure the document cover page',
  studentName: 'Student Name',
  institutionalAffiliation: 'Institutional Affiliation',
  courseName: 'Course Name',
  instructorName: 'Instructor Name',
  dueDate: 'Due Date',

  coverPageTab: 'Cover Page',
  coverDocTitle: 'Document Title',
  coverDocTitlePlaceholder: 'Ex: Analysis of convolutional neural networks...',
  coverSubtitle: 'Subtitle',
  coverSubtitlePlaceholder: 'Optional subtitle',
  coverAuthors: 'Author(s)',
  coverAuthorsPlaceholder: 'Ex: John Doe, Jane Smith',
  coverAuthorsHint: 'Separate multiple authors with commas',
  coverInstitution: 'Institution / University',
  coverInstitutionPlaceholder: 'Ex: National University',
  coverFaculty: 'Faculty / Department',
  coverFacultyPlaceholder: 'Ex: Department of Computer Science',
  coverCourse: 'Course / Subject',
  coverCoursePlaceholder: 'Ex: Database Fundamentals - Group 01',
  coverTeacher: 'Instructor',
  coverTeacherPlaceholder: 'Ex: Dr. Carlos Rodriguez',
  coverCity: 'City',
  coverCityPlaceholder: 'Ex: New York',
  coverDate: 'Date',
  includeCoverPage: 'Include cover page in export',
  includeCoverPageDesc: 'It will be added as the first page of the document',
  disableCoverPage: 'Disable cover page',
  enableCoverPage: 'Enable cover page',
  resetFields: 'Reset fields',
  institutionalLogo: 'Document Logo',
  uploadLogo: 'Upload Logo',
  replaceLogo: 'Replace Logo',
  deleteLogo: 'Delete Logo',
  suggestionLogo: 'It is recommended to use a logo without a background.',
  ieeeCoverNotice: 'IEEE cover page uses uppercase title and does not include course or instructor fields.',
  apa7CoverNotice: 'APA 7th Ed. format: title in upper half of page, author information centered, double spaced (§2.3).',
  apa6CoverNotice: 'APA 6th Ed. format: cover with centered title, author, institution, course, instructor and date.',
  fillFieldsToPreview: 'Fill in the fields to see the preview',
  apaHint: 'Title: upper half · Pg. 1 in header',
  selectDate: 'Select date…',
  clearDate: 'Clear date',
  prevMonth: 'Previous month',
  nextMonth: 'Next month',
  datePickerLabel: 'Date picker',
  goToToday: 'Go to today',
  months: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
  daysShort: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
  dateLocaleFormat: 'en-US',

  // Page Settings
  titleSettingsTab: 'Settings',
  pageSettingsTitle: 'Page Settings',
  pageNumbering: 'Page numbering',
  pageNumberPosition: 'Numbering position',
  posTopRight: 'Top right',
  posTopCenter: 'Top center',
  posTopLeft: 'Top left',
  posBottomRight: 'Bottom right',
  posBottomCenter: 'Bottom center',
  posBottomLeft: 'Bottom left',
  pageNumberDefaultFormat: 'By default, depends on the selected format ({0}).',
  startNumberOnCover: 'Start numbering on cover',
  startNumberOnCoverDesc: 'If turned off, the first page will not have a number and the second page will start at 2.',

  // Table of Contents
  tocToggle: 'Generate table of contents',
  tocToggleDesc: 'Automatically inserts a table of contents before the document body.',
  tocSection: 'Table of contents',
  tableOfContents: 'Table of Contents',
  tocNoHeadings: 'No valid headings found. Add headings (Heading 1, Heading 2 or Heading 3) to the document to generate a table of contents.',
  tocPage: 'Pg.',

  nd: 'n.d.',
  etAl: 'et al.',
  retrievedFrom: 'Retrieved from',
  referencesHeading: 'References',
  unknownAuthor: 'Unknown author',
  unknownTitle: '[Title]',
  incompleteReferenceFallback: 'Incomplete reference',

  // Support Form
  supportTitle: 'Support and Feedback',
  supportSpamWarning: 'To avoid spam, you can send a new message in {0} minute(s).',
  supportError: 'There was a problem sending your message. Please try again later.',
  supportType: 'Request type',
  supportDescription: 'Description',
  supportDescriptionPlaceholder: 'Tell us more about it...',
  supportSending: 'Sending...',
  supportSend: 'Send',
  supportSuccessTitle: 'Message sent!',
  supportSuccessMsg: 'Thank you for helping us improve the platform.',
  supportReqBug: 'Bug report',
  supportReqSupport: 'Support request',
  supportReqSuggestion: 'Improvement suggestion',
  supportReqFeature: 'Feature request',
  supportReqGeneral: 'General comment',
  supportReqOther: 'Other',

  // Compliance Modal
  complianceEvalTitle: 'Compliance Evaluation',
  complianceEvalNorm: 'Evaluated Norm: {norm}',
  complianceEvalScore: 'Current compliance',
  complianceElementsOk: 'Compliant elements:',
  complianceElementsMissing: 'Missing elements:',
  complianceElementsWarning: 'Warnings:',
  close: 'Close',
  reviewMyDocument: 'Review My Document',
  normalizeBtn: 'Auto Normalize',

  // Normalization Missing
  missingForNormTitle: 'Missing items to apply norms',
  missingForNormDesc: 'The following structural elements must be completed to enable automatic normalization:',
  missingNormTitle: 'Set a main title.',
  missingNormHeadings: 'Define heading levels.',
  missingNormRefs: 'Add bibliographic references.',
  missingNormCover: 'Complete the cover page.',
  missingNormCitations: 'Incorporate in-text citations.',

  // Engine IRule Names (from API)
  'Portada incompleta': 'Incomplete Cover Page',
  'Portada': 'Cover Page',
  'Fuente permitida': 'Allowed Font',
  'Referencias bibliográficas': 'Bibliographic References',
  'Interlineado doble': 'Double Spacing',
  'Márgenes correctos': 'Correct Margins',
  'Jerarquía de encabezados': 'Heading Hierarchy',

  // APA6-specific i18n keys
  'apa6.font.name': 'Allowed Font (APA 6)',
  'apa6.font.desc': 'APA 6 requires Times New Roman at 12pt.',
  'apa6.font.error': 'Times New Roman 12pt font was not detected.',

  // IEEE i18n keys
  'ieee.general.name': 'General IEEE Verification',
  'ieee.general.desc': 'Base heuristic verification for IEEE format.',
  'ieee.general.twoCol': 'Two-column layout',
  'ieee.general.warning': 'Two-column layout cannot be verified from extracted text.',

  // CrossReferenceRule i18n keys
  'crossRef.name': 'Citations/References Match',
  'crossRef.desc': 'Verifies that every citation has a reference and every reference has been cited.',
  'crossRef.ok': 'All citations and references match correctly.',
  'crossRef.bothMissing': 'There are in-text citations without a reference and references without citations.',
  'crossRef.orphanCitations': 'There are in-text citations that have no corresponding reference in the list.',
  'crossRef.unusedRefs': 'There are references that have not been cited in the text: ',
  'crossRef.empty': 'No citations or references found to cross-reference.',
  'crossRef.untitled': 'Untitled',

  // Engine IRule Messages (from API)
  'Se mencionan referencias, pero no parece haber una sección con encabezado formal.': 'References are mentioned, but no formal section heading was found.',
  'No se encontró la sección de Referencias.': 'References section was not found.',
  'Se encontró un encabezado de nivel 2 sin un nivel 1 previo.': 'A level 2 heading was found without a preceding level 1 heading.',
  'No se encontraron encabezados en el documento.': 'No headings were found in the document.',
  'Se detectó una fuente no recomendada (ej. Comic Sans).': 'A non-recommended font was detected (e.g. Comic Sans).',
  'El documento es demasiado corto o no tiene portada clara.': 'The document is too short or has no clear cover page.',
  'No se pudo verificar claramente la estructura de la portada.': 'The cover page structure could not be clearly verified.',
  'Referencia sin título': 'Untitled reference',

  // Figures
  insertFigure: 'Insert Figure',
  figureTitle: 'Figure Title',
  figureCaption: 'Caption',
  figureNote: 'Note',
  figureImage: 'Image',
  figureBasicInfo: 'Basic Information',
  figureApaAttribution: 'APA Attribution',
  figureOriginalTitle: 'Original image title',
  figureAuthor: 'Original author',
  figureYear: 'Publication year',
  figureSource: 'Original source (e.g. website, book)',
  figureUrl: 'URL',
  figureLicense: 'License (e.g. CC BY 4.0)',
  figureImageRequired: 'An image is required.',
  figureTitleRequired: 'A title is required.',
  figure: 'Figure',
  noteAdaptedFrom: 'Note. Adapted from',
  noteFrom: 'Note. From',
  figuresTab: 'Figures',
  figuresTabEmpty: 'No figures in the document.',
  figuresTabUnselected: 'Select a figure in the editor to modify its properties.',
};
