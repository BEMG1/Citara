...
export const ExportPDFProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const executePdfExport = useCallback(async () => {
    ...
    try {
      ...
      await exportToPdf(documentText, references, suggestedName, formatter, language, coverPage, pageNumberPosition, startNumberingOnCover, generateTOC, citationFormat, documentStyle);
